...
var Order = {
    ...
    removeOption (optionId) {
        let optionIndex = findIndex(Order.options, (item) => {
    ...
    getRoute: function () {
        var route = this._addresses_list
            .filter((item, index) => { if( item.isFull() ) { return true }})
            .map((item, index) => {return item.currentAddress.address.point});
    ...
    getFullRoute() {
        return this._addresses_list
            .filter((item, index) => { if( item.isFull() ) { return true }})
            .map((item, index) => {return item.currentAddress.address});
    },
